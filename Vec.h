...
public:
  ...
  decltype(sqrt(T())) magnitude() const { return sqrt(magnitude_square()); }
...
public:
  ...
  void rotate(const Vec<T, 3> &axis, T theta) { *this = rotation(axis, theta); }
  ...
  Vec reflect(const Vec<T, 3, C> &normal) { *this = reflection(normal); }
