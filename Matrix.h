...
template <typename T, size_t N, bool C> class Vec;
...
template <typename T, size_t M, size_t N = M> class Matrix {
  ...
  public:
    ...
    template <typename U> Matrix(const Matrix<U, M, N> &other) {
        static_assert(M > 0 && N > 0, "Matrix cannot have a zero dimension");
    ...
    std::array<T, N> &operator[](size_t i) { return v[i]; }

    const std::array<T, N> &operator[](size_t i) const { return v[i]; }
...
// This is really slow for large matrices (and would also be very prone
// to stack overflows)
template <typename T, size_t N> T _determinant(const Matrix<T, N> &mat) {
    T rval(0);
    bool sign = true;
...
template <typename T> T _determinant(const Matrix<T, 1> &mat) {
    return mat[0][0];
}
